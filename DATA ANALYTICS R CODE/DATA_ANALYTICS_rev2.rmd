...
# train the model
fsmodel <- train(target~., data=heart, method="lvq", preProcess="scale", trControl=control)
# estimate variable importance
importance <- varImp(fsmodel, scale=FALSE)
...
## we can see that only cp and thalach has the highest importance on prediciting the outcome with each having an importance of 0.75.
## oldpeak and ca are close behind cp and thalach and have importance in the range of 72 to 74.
...
results <- rfe(heart[,1:13], heart[,14], sizes=c(1:13), rfeControl=control)
...
```
## from this graph we can infer that when we are using 3 attribute/variables we get average accuracy
## compared to the base model.

## when we use 9 variable we are able to get comparable accuracy compared to that of base model

## When we use all 13 variables we get the base model with highest accuracy.
